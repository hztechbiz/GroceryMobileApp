...
class SwiperBanner extends PureComponent {
  ...
  async getOneProduct(value) {
    ...
    formData.append('currency_code', this.props.currency);
    ...
    if (json2.success !== '1') {
      ...
      this.refs.toast.show(json2.message + this.props.language);
    } else {
      this.setState({SpinnerTemp: false});
...
const getLanguage = (state) => state.Config.languageJson['No Products Found'];
...
const getCurrenctFun = createSelector([getCurrency], (getCurrency) => {
  return getCurrency;
});
const getBannersFun = createSelector([getKeyword], (getKeyword) => {
  return getKeyword;
});
