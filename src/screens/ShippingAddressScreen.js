...
class ShippingAddress extends Component {
  ...
  onTextChange = (number) => {
    if (this.state.shippingData.phonenumber === undefined) {
      this.state.shippingData.phonenumber = '';
    }
    const num = parsePhoneNumberFromString(
      number,
      CountryCodeKey[this.state.shippingCountryCode][0],
    );
    const reg = /^[0-9]/;
    if (
      !!num &&
      this.state.shippingData.phonenumber.toString().length > number.length &&
      !reg.test(
        this.state.shippingData.phonenumber[
          this.state.shippingData.phonenumber.toString().length - 1
        ],
      )
    ) {
      ...
      this.state.shippingData.phonenumber = new AsYouType(
        CountryCodeKey[this.state.shippingCountryCode][0],
      ).input(number);
      this.setState({shippingData: this.state.shippingData});
    }
  ...
  onTextChange2 = (number) => {
    ...
    const num = parsePhoneNumberFromString(
      number,
      CountryCodeKey[this.state.billingCountryCode][0],
    );
    const reg = /^[0-9]/;
    if (
      !!num &&
      this.state.billingArray.phonenumber.toString().length > number.length &&
      !reg.test(
        this.state.billingArray.phonenumber[
          this.state.billingArray.phonenumber.toString().length - 1
        ],
      )
    ) {
      let phone = num.nationalNumber.split('');
      ...
      this.state.billingArray.phonenumber = new AsYouType(
        CountryCodeKey[this.state.billingCountryCode][0],
      ).input(number);
      this.setState({billingArray: this.state.billingArray});
    }
  ...
  phoneNumberCheck() {
    const reg = /^([0-9\\(\\)\\/\\+ -]*)$/;
    return (
      this.state.billingArray.phonenumber.length >= 11 &&
      reg.test(this.state.billingArray.phonenumber) === true
    );
  }
  ...
  refresh = (name, selectedValue, id, temp) => {
    if (temp === 'country') {
      ...
      this.state.shippingData.zone_name =
        SyncStorage.get('tempdata').entry_zone;
    }
    if (temp === 'zone') {
  ...
  phoneNumberCheckshipping() {
    const reg = /^([0-9()/+ -]*)$/;
    return (
      (String(this.state.shippingData.phonenumber).length >= 10 ||
        String(this.state.shippingData.phonenumber).length === 0) &&
      reg.test(String(this.state.shippingData.phonenumber)) === true
    );
  }
  ...
  phoneNumberCheckbillingArray() {
    const reg = /^([0-9()/+ -]*)$/;
    return (
      (String(this.state.billingArray.phonenumber).length >= 10 ||
        String(this.state.billingArray.phonenumber).length === 0) &&
      reg.test(String(this.state.billingArray.phonenumber)) === true
    );
  }
              ...
              <View style={{width: WIDTH * 0.7}}>
                ...
                <View
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      this.setState(
                        {spinnerTemp: true, invalidCode: false},
                        () => {
                ...
                {this.state.invalidCode ? (
                  <Text
                    ...
                    {this.props.cartItems2.Config.languageJson2['Invalid Code']}
                  </Text>
                ) : null}
              </View>
            </DialogContent>
