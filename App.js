...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.cartItems2.Config.appInProduction) {
      theme.bottomNavigation =
        SyncStorage.get('bottom') !== undefined
          ? SyncStorage.get('bottom')
          : false;

      theme.backgroundColor =
        SyncStorage.get('backgroundColor') !== undefined
          ? SyncStorage.get('backgroundColor')
          : '#fdfdfd';

      theme.textContrast =
        SyncStorage.get('textContrast') !== undefined
          ? SyncStorage.get('textContrast')
          : '#000000';

      theme.textColor =
        SyncStorage.get('textColor') !== undefined
          ? SyncStorage.get('textColor')
          : '#000000';
    } else {
  ...
  getRecentProducts = async () => {
    const data = {};
    if (SyncStorage.get('customerData').customers_id != null) {
      data.customers_id = SyncStorage.get('customerData').customers_id;
    }
    data.language_id =
  ...
  wishlistData: (props, th) => {
    th.setState({SpinnerTemp: true}, () => {
      dispatch(async (dispatch) => {
        ...
        if (json.success === '1') {
          var prod =
            json.product_data.length === 0 ? undefined : json.product_data;
          dispatch({
